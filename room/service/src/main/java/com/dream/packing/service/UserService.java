...
public class UserService implements UserServer {
			...
			try {
				...
				if (!isExist) {
					...
					if (userMap == null || userMap.size() == 0) {
						...
						List<Map<String, Object>> rolesList = userRoleDao.findRoleByUserId((Integer)userMap.get("id"));
						
						StringBuffer roleBuffer = new StringBuffer();
						
						for (Map<String, Object> map : rolesList) {
							roleBuffer.append(',').append(map.get("id").toString());
						}
						
						List<Map<String, Object>> powersList = rolePowerDao.findPowers(roleBuffer.deleteCharAt(0).toString());
						
						userMap.put("roleList", rolesList);
						userMap.put("powerList", powersList);
				...
				throw new RollBackException("登录失败", e);
			}
		}
		return messageMap;
	}
	...
	public Map<String, Object> addUser(Map<String, Map<String, Object>> paramsMap) {
		...
		if (paramsMap == null || paramsMap.size() == 0) {
			...
			if (userMap == null || userMap.size() == 0 || personMap == null || personMap.size() == 0) {
			...
			} else if(paramsMap.get("userRoles") == null) {
				messageMap.put("message", "请分配角色！");
			} else {
				...
				try {
					...
					List<Map<String, Object>> listMap = (List<Map<String, Object>>)paramsMap.get("userRoles");
					
					userRoleDao.bacthRole(listMap);
	...
	@Transactional
	@SuppressWarnings("unchecked")
	public Map<String, Object> updateUser(Map<String, Map<String, Object>> paramsMap) {
		messageMap.put("result", "failure");
		messageMap.put("data", null);
		...
		if (paramsMap == null || paramsMap.size() == 0) {
			...
			if (userMap == null || userMap.size() == 0 || personMap == null || personMap.size() == 0) {
				...
				try {
					...
					messageMap.put("message", "添加失败，请稍后再试！");
					throw new RollBackException("用户修改失败", e);
				}
			}
		}
		return messageMap;
